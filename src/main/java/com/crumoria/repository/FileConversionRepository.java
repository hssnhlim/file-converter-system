...
public interface FileConversionRepository extends JpaRepository<FileConversion, UUID> {
    ...
    List<FileConversion> findByStatus(ConversionStatus status);
    ...
    List<FileConversion> findByUserIdAndCreatedAfter(@Param("userId") UUID userId, @Param("since") java.time.LocalDateTime since);
    
    @Query("SELECT COUNT(fc) FROM FileConversion fc WHERE fc.userId = :userId AND fc.status = :status")
    long countByUserIdAndStatus(@Param("userId") UUID userId, @Param("status") ConversionStatus status);
    
    @Query("SELECT fc FROM FileConversion fc WHERE fc.status IN ('PENDING', 'PROCESSING') ORDER BY fc.createdAt ASC")
    List<FileConversion> findPendingAndProcessingConversions();
